...
public class LinearSolver {
	...
	public Matrix upperTriangular(Matrix coefficients, Matrix rightSide) {
		int rows = coefficients.countRows();
		int columns = coefficients.countColumns();
		Matrix augmented = coefficients.augmentColumn(rightSide);
		
		// get 0's below each pivot
		for (int j = 0; j < columns; j++) {
			// if 0 at pivot position, swap or move on to next column
			if (Math.abs(augmented.getValue(j, j)) <= error){
				for (int i = j + 1; i < rows; i++) {
					// if a suitable pivot is found, do a row swap.
					if (Math.abs(augmented.getValue(i, j)) >= error){
						augmented = augmented.rowSwap(i, j);
						swaps.put(i, j);
					}
				}
			} else{	// if no row swaps needed, proceed as normal for this column
				for (int i = j + 1; i < rows; i++){
					double multiplier = augmented.getValue(i, j) / augmented.getValue(j, j);
					Matrix elementary = Matrix.Identity(rows);
					elementary.setValue(-multiplier, i, j);
	...
	public Matrix backSubstitute(Matrix Ab) {
		...
		for (int i = components - 1; i >= 0; i--) {
			...
			if (Math.abs(Ab.getValue(i, i)) >= error) {
